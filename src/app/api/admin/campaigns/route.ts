...
export async function GET(request: NextRequest) {
  ...
  const items = await getAllCampaigns(auth.tenantId)
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = campaignSchema.partial().safeParse(fields)
  if (!parsed.success) {
    return NextResponse.json({ error: 'Doğrulama hatası', details: parsed.error.flatten().fieldErrors }, { status: 422 })
  }

  const supabase = createAdminClient()
  const { error } = await supabase.from('campaigns').update(parsed.data).eq('id', id).eq('tenant_id', auth.tenantId)
...
export async function DELETE(request: NextRequest) {
  ...
  const supabase = createAdminClient()
  const { error } = await supabase.from('campaigns').delete().eq('id', id).eq('tenant_id', auth.tenantId)
